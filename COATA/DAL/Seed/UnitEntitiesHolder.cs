...
namespace DAL.Seed
{
    public class UnitEntitiesHolder
    {
        ...
        {
            List<UnitTree> unitTrees = new List<UnitTree>();
            foreach (var unit in _unitList)
            {
                ...
                {
                    int nodeId = _unitList.FirstOrDefault(x=>x.Id == unit.ParentId).Rgt;
                    foreach (var unitTree in _unitList.Where(x=>x.Rgt>=nodeId))
                    {
                        unitTree.Rgt+=2;
                    }
                    foreach (var unitTree in _unitList.Where(x=>x.Lft>nodeId))
                    {
                        unitTree.Lft+=2;
                    }
                    unit.Lft = nodeId;
