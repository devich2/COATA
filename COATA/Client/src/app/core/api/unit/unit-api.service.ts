...
@Injectable() export class UnitApiService {
  ...
  expand(unitId: number, classificationId: number): Observable<UnitModel[]> {
    ...
    return this.httpClient.get<UnitModel[]>('/api/unit/expand', {
      params
    });
  }
  ...
  expandGrouped(unitId: number = null): Observable<SelectionUnitModel> {
    ...
    if (unitId != null) params = params.append("unitId", unitId.toString());
    return this.httpClient.get<SelectionUnitModel>('/api/unit/expand_grouped', {
      params
    });
  }
  ...
  search(name: string, unitType: string): Observable<SelectionUnitModel> {
    ...
    return this.httpClient.get<SelectionUnitModel>('/api/unit/search', {
      params
    });
  }
  ...
  searchExpanded(name: string, unitType: string): Observable<SelectionUnitModel> {
    ...
    return this.httpClient.get<SelectionUnitModel>('/api/unit/search_expanded', {
      params
    });
  }
