import {
  ...
  export class UnitApiService {
    ...
    expandGrouped(unitId:number): Observable<SelectionUnitModel> {
      const params = new HttpParams({
        fromObject: {unitId: unitId.toString()}
      })
    ...
    search(name:string, unitType: string): Observable<SelectionUnitModel> {
      const params = new HttpParams().append("name", name).append("unitType", unitType);
